...
function Heading() {
  ...
  const linkStyle =
    "text-gray-700 font-medium hover:text-teal-600 transition";
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[1000] bg-white border-b border-gray-200">
      ...
      {/* Mobile Nav Overlay */}
      {isMenuOpen && (
        <nav className="md:hidden absolute top-full left-0 right-0 bg-white border-t border-gray-200">
          <div className="flex flex-col px-6 py-6 gap-5 max-h-[70vh] overflow-y-auto">
            {links.map(link => (
              <a
                key={link.name}
                href={link.href}
                className="text-gray-700 text-base font-medium hover:text-teal-600"
